...
class AccessControlledInternalDataStore(object):
	...
	if probe == "LocationProbe":
            cursor = self.getDataInternal(probe, startTime, endTime)
	    if cursor:
                for entry in cursor:
	...
	try:
		...
		if probe == "LocationProbe":
	            ...
	            validLocations = self.checkLocation(startTime, endTime, contextObject, None)
	...
	if dataLocations == None:
	    dataLocations = set()
	    cursor = self.getData("LocationProbe", startTime, endTime)
	    if cursor:
	        for entry in cursor:
	            dataLocations.add((entry[u'value'][u'mlatitude'], entry[u'value'][u'mlongitude']))
